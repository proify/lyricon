...
- **前置条件**：设备需获取 **Root** 权限，并安装支持 libxposed API 101 或 102 的 **LSPosed**
  （或兼容的 Xposed）框架。

> [!TIP]
> 为保证功能稳定，建议使用明确支持 libxposed API 102 的框架版本；API 101 仍受支持。
...
4. **安装插件**：根据使用的音乐播放器，在 [LyricProvider](https://github.com/tomakino/LyricProvider)
   下载对应插件。
5. **参数调节**：进入词幕应用，根据实际屏幕情况调整位置锚点、宽度与视觉样式。为避免歌词与时间重叠，
   `clock` 默认会在歌词显示时隐藏；若需同时显示，请把它的视图规则设为“默认”。
